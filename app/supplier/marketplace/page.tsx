...
import { mockSupplierRFPs } from '@/lib/mock-supplier-rfps'
      ...
      <div className="border-t border-border px-5 py-3 flex items-center justify-between gap-2 bg-gray-50/70">
        ...
        <button
          ...
          className={cn(
            ...
            saved ? 'text-[#16A34A] bg-[#F0FDF4]' : 'text-text-muted hover:text-[#16A34A] hover:bg-[#F0FDF4]'
          )}
          aria-label={saved ? 'Remove bookmark' : 'Bookmark RFP'}
        >
...
function getStatusIcon(status: string) {
  switch (status) {
    case 'submitted': case 'awarded': return CheckCircle
    case 'draft': case 'in_progress': return Clock
...
function getStatusStyle(status: string) {
  switch (status) {
    case 'awarded': return 'bg-[#F0FDF4] text-[#166534] border-[#16A34A]/20'
    case 'submitted': return 'bg-blue-50 text-blue-700 border-blue-200'
    case 'in_progress': return 'bg-amber-50 text-amber-700 border-amber-200'
    case 'draft': return 'bg-gray-100 text-text-muted border-border'
    case 'rejected': return 'bg-red-50 text-red-700 border-red-200'
    default: return 'bg-gray-100 text-text-muted border-border'
  }
}
...
export default function SupplierMarketplacePage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
            ...
            <div className="flex items-center gap-4">
              <div>
                ...
                <p className="text-xs text-text-muted mt-0.5">
                  {marketView === 'internal'
                    ? `${mockSupplierRFPs.length} active bid${mockSupplierRFPs.length === 1 ? '' : 's'}`
      ...
      {/* My Bids (Supplier's own active RFP bids) */}
      {marketView === 'internal' && (
        <div className="px-6 py-8">
          {mockSupplierRFPs.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-24 text-center gap-3">
              <FileText className="h-10 w-10 text-text-muted" />
              <p className="text-sm font-medium text-text-primary">No active bids</p>
              <p className="text-xs text-text-muted">Browse the public marketplace to find opportunities</p>
              <Button size="sm" className="bg-[#16A34A] hover:bg-[#15803d] text-white mt-2" onClick={() => setMarketView('public')}>
                Browse Marketplace
              </Button>
            </div>
          ) : (
            <div className="space-y-4">
              {mockSupplierRFPs.map(rfp => {
                const days = getDaysUntil(rfp.deadline)
                const urgent = days <= 14
                const critical = days <= 7
                const StatusIcon = getStatusIcon(rfp.submissionStatus)

                return (
                  <div key={rfp.id} className="bg-white border border-border rounded-xl p-5 hover:shadow-md transition-shadow">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0 space-y-3">
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <Badge className={cn('text-[10px] border shrink-0', getStatusStyle(rfp.submissionStatus))}>
                            <StatusIcon className="h-3 w-3 mr-1" />
                            {getStatusLabel(rfp.submissionStatus)}
                          </Badge>
                        </div>
                        ...
                        <div className="flex flex-wrap items-center gap-4 text-xs text-text-muted">
                          ...
                          {rfp.completionPercent > 0 && rfp.submissionStatus !== 'submitted' && rfp.submissionStatus !== 'awarded' && (
                            <span className="flex items-center gap-1.5">
                              ...
                              <span className="text-text-muted">{rfp.completionPercent}% complete</span>
                            </span>
                          )}
                        </div>
