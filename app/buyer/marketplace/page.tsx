...
const buyerInternalRFPs: InternalRFP[] = [
  {
    id: '1',
...
export default function BuyerMarketplacePage() {
  ...
  const handleSave = (rfp: MarketplaceRFP) => {
    setSavedIds(prev => {
      ...
      if (s.has(rfp.id)) s.delete(rfp.id)
      else s.add(rfp.id)
      return s
    })
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
            ...
            <div className="flex items-center gap-4">
              <div>
                ...
                <p className="text-xs text-text-muted mt-0.5">
                  {marketView === 'internal'
                    ? `${buyerInternalRFPs.length} internal RFP${buyerInternalRFPs.length === 1 ? '' : 's'}`
      ...
      {/* My RFPs (Buyer's own created RFPs) */}
      {marketView === 'internal' && (
        <div className="px-6 py-8">
          {buyerInternalRFPs.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-24 text-center gap-3">
              <FileText className="h-10 w-10 text-text-muted" />
              <p className="text-sm font-medium text-text-primary">No RFPs created yet</p>
              <p className="text-xs text-text-muted">Create your first RFP to get started</p>
