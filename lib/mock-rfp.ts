...
/**
 * Convert RFPDocument to TenderData format for the list view
 */
export function rfpToTenderData(rfp: RFPDocument): {
  id: string;
  name: string;
  referenceId: string;
  status: 'draft' | 'open' | 'active' | 'evaluation' | 'closed';
  category: string;
  submissions: number;
  deadline: string;
  budget: number;
  ...
  return {
    id: rfp.id,
    name: rfp.title || rfp.projectInfo.projectName || 'Untitled RFP',
    referenceId: rfp.projectInfo.referenceId || `RFP-${rfp.id.slice(-6)}`,
    status: rfp.status === 'published' ? 'open' : rfp.status as 'draft' | 'open' | 'active' | 'evaluation' | 'closed',
    category: rfp.projectInfo.category || 'Uncategorized',
    submissions: 0,
    deadline: rfp.projectInfo.submissionDeadline 
      ? new Date(rfp.projectInfo.submissionDeadline).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })
      : 'No deadline set',
    budget: rfp.projectInfo.expectedBudget || 0,
    owner: rfp.projectInfo.team?.[0]?.name || 'Unassigned',
    isFromStore: true,
  };
}
